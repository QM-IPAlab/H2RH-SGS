...
def execute_commands(safe_grasp_folder, processed_frames, errors_file):
    ...
    with open('./processed_frames.txt', 'a') as pf:
...
def process_dataset(dataset_path, max_threads):
    ...
    if os.path.exists('./processed_frames.txt'):
        with open('./processed_frames.txt', 'r') as pf:
    ...
    with Pool(processes=max_threads) as pool:
        results = pool.starmap(execute_commands, [(task, processed_frames, './errors_frame.txt') for task in tasks])
