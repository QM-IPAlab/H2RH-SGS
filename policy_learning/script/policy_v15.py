...
def eular_R_cuda(euler_angles, translation):
    batch_size = euler_angles.shape[0]
    T = torch.eye(4, device='cuda').repeat(batch_size, 1, 1)  # (batch_size, 4, 4)

    for i in range(batch_size):
        R_np = transforms3d.euler.euler2mat(euler_angles[i, 0].item(), 
                                            euler_angles[i, 1].item(), 
                                            euler_angles[i, 2].item(), 'sxyz')
        R_torch = torch.tensor(R_np, dtype=torch.float32, device='cuda')  # 转换为 torch
        T[i, :3, :3] = R_torch  # 赋值旋转矩阵
        T[i, :3, 3] = translation[i]  # 赋值平移向量
