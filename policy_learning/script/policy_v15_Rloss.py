...
class PoseLoss(nn.Module):
    ...
    def forward(self, pred_pose, pred_class, target_pose, target_class):
        ...
        for i in range(batch_size):
            pred_euler = pred_pose[i, :3]  # 旋转部分
            pred_translation = pred_pose[i, 3:]   # 平移部分
            target_euler = target_pose[i, :3]  # 旋转部分
            target_translation = target_pose[i, 3:]  # 平移部分

            R_pred = eular_R(pred_euler,pred_translation)[:3,:3]
            R_target = eular_R(target_euler,target_translation)[:3,:3]
            
            
            R_diff = torch.norm(R_pred - R_target,p=2)

            T_diff = torch.norm(pred_translation - target_translation, p=2)
            
            rot_loss += R_diff
            trans_loss += T_diff
            
        rot_loss = rot_loss / batch_size
        trans_loss = trans_loss / batch_size
        class_loss = self.mse_loss(pred_class, target_class)
        total_loss = score_r*rot_loss + score*trans_loss + class_loss
...
def train_and_evaluate(model, train_dataloader, test_dataloader, criterion, optimizer, epochs=10, device='cuda',model_path='./best_pose_estimation_model_v10_0209.pth'):
    ...
    for epoch in range(epochs):
        ...
        avg_total_test_loss,avg_rot_test_loss,avg_test_trans_loss, avg_class_loss = evaluate(model, test_dataloader, criterion, device,'train',log_file)
