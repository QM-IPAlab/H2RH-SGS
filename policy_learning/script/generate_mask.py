...
def get_mask(pose_file, hand_ply_file, object_pcl_file,output_dir):
    ...
    hand_pil = Image.open(fill_hand_mask_save_path).convert('RGB').convert('L')
    object_pil = Image.open(fill_object_mask_save_path).convert('RGB').convert('L')
...
def get_mask_img(pose_file, hand_ply_file, object_pcl_file,output_dir):
    """
    Generate projected images for hand and object point clouds based on a given pose.
    ...
    hand_img_np = np.where(hand_img_np > threshold, 255, 0).astype(np.uint8)
    mask_filled_h = fill_mask(hand_img_np)
    # mask_filled_h = hand_img_np
   
    object_img = object_pcl.project_to_depth_image(width, height, intrinsic_tensor,  pose)
    object_img_np = np.asarray(object_img.to_legacy())
    object_img_np = np.where(object_img_np > threshold, 255, 0).astype(np.uint8)
    mask_filled_o = fill_mask(object_img_np)
    # mask_filled_o = object_img_np
    
    # Create a color mask image (RGB)
    mask_combined = np.zeros((height, width, 3), dtype=np.uint8)
    mask_combined[mask_filled_h == 255] = [255,0,0]#[255, 192, 203]    # Red for hand
    mask_combined[mask_filled_o == 255] = [0, 255, 0]    # Green for object
    mask_combined[(mask_filled_h == 255) & (mask_filled_o == 255)] = [0, 255, 0]  # Yellow for overlap
    
    # Save the combined mask
    combined_mask_path = os.path.join(output_dir, 'hand_object_mask.png')
    plt.imsave(combined_mask_path, mask_combined)
    print(f"Combined mask saved at {combined_mask_path}")
    
def mv_background(img, hand_mask, object_mask,save_path):
   
    
    # 转换为numpy数组
    img_array = np.array(img)
    hand_mask_array = np.array(hand_mask)
    object_mask_array = np.array(object_mask)
    
    # 生成合并的mask，确保mask是二值的（0 或 255）
    combined_mask = ((hand_mask_array > 0) | (object_mask_array > 0)).astype(np.uint8) * 255

    
    # 扩展mask到RGB通道
    combined_mask = np.stack([combined_mask] * 3, axis=-1)
    
    img_no_background = np.where(combined_mask, img_array, 0)

 
    
    # 转换回PIL图像并保存
    img_no_background = Image.fromarray(img_no_background.astype(np.uint8))
    # os.makedirs(save_path, exist_ok=True)
    # save_path = os.path.join(save_path,'no_background_img.png')
    # img_no_background.save(save_path, format='PNG')
...
hand_mask, object_mask = get_mask(pose, hand_ply_file, object_pcl_file, output_dir)
img =  Image.open(img_path).convert("RGB")

img_no_back_ground = mv_background(img, hand_mask, object_mask, output_dir)